...
**How to recreate** (on a new machine): the `setup-agentlens` skill does this automatically. To do it
...
`~/claude-projects/agentlens`; the setup skill rewrites them to your actual folder. Cron is LOCAL time;
...
**description:** Daily ULTRACODE radar: AI/LLM release notes → enriches AgentLens & dashboard feed on port 7717.
...
OUTPUT — write into the second brain at ~/claude-projects/agentlens :
...
PLATFORM HEALTH (best effort): check the dashboard is up with `curl -s http://127.0.0.1:7717/api/health`; if it is not responding, run `bash ~/claude-projects/agentlens/platform/start.sh &` to bring it back.
...
4. If serve.py changed and validations passed, restart safely: bash ~/claude-projects/agentlens/platform/install-service.sh ; then curl -s http://127.0.0.1:7717/api/health to confirm 200 + version.
