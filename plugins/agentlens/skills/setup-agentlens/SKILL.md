---
name: setup-agentlens
description: One-run setup for the AgentLens dashboard + second brain. Installs the always-on local service on port 7717, seeds a persistent brain folder from the plugin bundle, registers the scheduled ULTRACODE jobs, wires memory, and verifies everything. Use when the user says "set up AgentLens", "set up the AgentLens dashboard", "install AgentLens", or runs this skill after installing the AgentLens plugin.
when_to_use: A user has installed the AgentLens plugin and wants to stand up the dashboard + second brain on their Mac.
---

# setup-agentlens (prescriptive, one-run)

Goal: take the installed **AgentLens plugin** and stand up the entire capability — a persistent
...
- **BRAIN** = the persistent brain folder. Default `~/claude-projects/agentlens`. If the user already has
  a AgentLens folder (e.g. one connected to Cowork) prefer that and confirm it with them.
...
This generates a LaunchAgent (label `com.agentlens.dashboard`) with BRAIN's absolute path (starts at login
...
`taskId`, `cronExpression`, `description`, and `prompt` — but FIRST replace any `~/claude-projects/agentlens`
...
The three jobs: `ai-release-radar` (daily 01:00), `ai-deep-dive-weekly` (Sun 01:00). (The `dashboard-optimizer` nightly engine ships in AgentLens Pro.)
...
- Append an init line to `<BRAIN>/logs/research-log.md`: "<DATE> — brain initialised for <user> by setup-agentlens".
