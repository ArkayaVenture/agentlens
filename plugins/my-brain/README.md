# 🧠 my-brain — Claude Code / Cowork plugin
...
2. Run the **`setup-my-brain`** skill — or say *"set up the my-brain dashboard"*. It seeds a persistent
   brain folder (default `~/claude-projects/my-brain`), installs the always-on service on port 7717,
...
`bash ~/claude-projects/my-brain/platform/ctl.sh {start|stop|restart|status|logs}`.

## What's in the bundle

| Path | Purpose |
|------|---------|
| `skills/setup-my-brain/` | One-run installer skill (entry point). |
...
`MYBRAIN_PORT` (7717) · `MYBRAIN_LIVE_WINDOW` (900s) · `MYBRAIN_DASHBOARD` ·
`MYBRAIN_CLAUDE_PROJECTS` · `MYBRAIN_COWORK_ROOT` · `MYBRAIN_CURSOR_ROOT` ·
`MYBRAIN_FIRM_MCP` (comma list of MCP servers to classify as "firm/internal").

## Uninstall the service

```
launchctl bootout gui/$(id -u)/com.mybrain.dashboard
rm ~/Library/LaunchAgents/com.mybrain.dashboard.plist
```
