# my-brain platform — local, agent-managed, on a dedicated port
...
```bash
bash ~/claude-projects/my-brain/platform/install-service.sh
# installs the LaunchAgent, starts it, opens http://127.0.0.1:7717/
```

Just once, in the foreground (no autostart):
```bash
bash ~/claude-projects/my-brain/platform/start.sh   # → http://127.0.0.1:7717/
```

Health check: `curl -s http://127.0.0.1:7717/api/health`.
Stop/uninstall: `launchctl bootout gui/$(id -u)/com.mybrain.dashboard ; rm ~/Library/LaunchAgents/com.mybrain.dashboard.plist`.
...
Each session is tagged with its `source` and a `live` flag (touched within `MYBRAIN_LIVE_WINDOW`, default 300s).
...
```
            ...
            └───────────────▲───────────────────────────────┘
                            │ reads (no-cache)
            ┌───────────────┴───────────────────────────────┐
            │  my-brain/ (file-based knowledge + control)    │
