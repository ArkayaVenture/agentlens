#!/usr/bin/env bash
# my-brain — portable, one-shot installer for team distribution.
...
BRAIN="$(cd "$HERE/.." && pwd)"
LABEL="com.mybrain.dashboard"
PORT="${MYBRAIN_PORT:-7717}"
U="$(id -u)"
LA="$HOME/Library/LaunchAgents"
PLIST="$LA/$LABEL.plist"

echo "→ my-brain bootstrap  (brain: $BRAIN)"
...
case "$BRAIN" in
  "$HOME/Downloads"/*|"$HOME/Desktop"/*|"$HOME/Documents"/*)
    ...
    echo "    The always-on service may be denied file access there. Move it to e.g. ~/claude-projects/my-brain." ;;
esac
...
<plist version="1.0"><dict>
  ...
  <key>EnvironmentVariables</key><dict><key>MYBRAIN_DASHBOARD</key><string>$BRAIN/dashboard</string></dict>
...
NEXT — finish setup inside Claude Cowork (it can't be done from the shell):
  Open Cowork on this folder and run the "setup-my-brain" skill (or say:
  "set up the my-brain dashboard"). It will register the 3 scheduled jobs,
