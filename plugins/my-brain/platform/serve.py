#!/usr/bin/env python3
"""
my-brain platform server  ·  dedicated local port (default 7717)
...
Env / flags:
  --port / MYBRAIN_PORT             (default 7717)
  MYBRAIN_CLAUDE_PROJECTS           (default ~/.claude/projects)
  MYBRAIN_COWORK_ROOT               (default ~/Library/Application Support/Claude/local-agent-mode-sessions)
  MYBRAIN_CURSOR_ROOT               (default ~/.cursor/projects)
  MYBRAIN_DASHBOARD                 (default ../dashboard)
  MYBRAIN_LIVE_WINDOW               (seconds; a session is "live" if touched within this; default 300)
...
DASHBOARD_DIR = os.path.normpath(os.environ.get("MYBRAIN_DASHBOARD", os.path.join(ROOT, "..", "dashboard")))
PROJECTS_DIR = os.path.expanduser(os.environ.get("MYBRAIN_CLAUDE_PROJECTS", "~/.claude/projects"))
COWORK_DIR = os.path.expanduser(os.environ.get("MYBRAIN_COWORK_ROOT", "~/Library/Application Support/Claude/local-agent-mode-sessions"))
CURSOR_DIR = os.path.expanduser(os.environ.get("MYBRAIN_CURSOR_ROOT", "~/.cursor/projects"))
SOURCES = [
...
LIVE_WINDOW = int(os.environ.get("MYBRAIN_LIVE_WINDOW", "900"))  # a session is "live/active" if touched within 15 min
...
FIRM_MCP = set(x.strip().lower() for x in os.environ.get("MYBRAIN_FIRM_MCP", _DEFAULT_FIRM).split(",") if x.strip())
...
def classify_tool(name, summary=""):
    ...
    if srv:
        ...
        if s in WEB_MCP:
            return "web"
        return "firm"  # unknown MCP defaults to firm/internal connector (override via MYBRAIN_FIRM_MCP)
...
def overview():
    ...
    return {
        ...
        "subagents": subs, "toolCalls": total_tools, "live": live, "bySource": by_source, "version": VERSION,
...
def graph():
    rows = sessions_enriched()
    nodes = [{"id": "root", "type": "root", "label": "my-brain", "meta": {"sessions": len(rows)}}]
    edges = []; seen = set()
    for r in rows:
        p = r["project"]; pid = "p:" + p
        if pid not in seen:
    ...
    def do_GET(self):
        try:
            ...
            if p.startswith("/api/"):
                ...
                if p == "/api/health":
                    return self._json({"status": "ok", "service": "my-brain", "version": VERSION,
                                       "time": datetime.datetime.now().isoformat(timespec="seconds"),
                                       "dashboardDir": DASHBOARD_DIR, "projectsDir": PROJECTS_DIR,
                                       "projectsExist": os.path.isdir(PROJECTS_DIR),
                                       "features": ["tokens", "provenance4", "multisource", "live", "tokenAttribution"],
...
def main():
    ...
    ap.add_argument("--port", type=int, default=int(os.environ.get("MYBRAIN_PORT", PORT_DEFAULT)))
    args = ap.parse_args()
    os.chdir(DASHBOARD_DIR)
    socketserver.TCPServer.allow_reuse_address = True
    handler = functools.partial(Handler, directory=DASHBOARD_DIR)
    with socketserver.TCPServer(("127.0.0.1", args.port), handler) as httpd:
        print(f"my-brain → http://127.0.0.1:{args.port}/")
