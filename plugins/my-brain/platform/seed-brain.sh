#!/usr/bin/env bash
# Seed (or refresh) a persistent my-brain folder from the plugin bundle.
#
# The plugin install directory may be read-only or get replaced on update, so the
# live brain lives in a stable, writable location (default ~/claude-projects/my-brain).
...
#   DEST = where the live brain should live (default ~/claude-projects/my-brain)
set -euo pipefail
SRC="${1:?usage: seed-brain.sh <SRC> [DEST]}"
DEST="${2:-$HOME/claude-projects/my-brain}"
SRC="$(cd "$SRC" && pwd)"
mkdir -p "$DEST"

echo "→ seeding my-brain"
echo "  from: $SRC"
echo "  into: $DEST"
